...
class FirstConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in xrange(self.M):
			#index start after num of conv layer
            idx = self.L + i + 1
            h = blocks['h' + str(idx - 1)]
			# fron conv to fully-connected layer, we have to reshape it to 1 row
            if i == 0:
        ...
        reg_loss = 0
		# much easier to compute regularization
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
	...
	# Hidden Affine layer
    # Size of the parameter (F*Hp*Wp,Hn
    # Input: (N,F*Hp*Wp)
    # Output: (N,Hh)
    ...
    b3 = np.zeros(Hc)
	# pay attention to how to update the dict in a more easier way!!!
    ...
    if self.use_batchnorm:
		...
		# running_mean,running_varis is equal to the number of filter in that layer
