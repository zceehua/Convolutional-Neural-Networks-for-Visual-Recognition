...
def affine_forward(x, w, b):
  ...
  # array([  2.,  12.])
  N=x.shape[0]
  x_reshape=x.reshape(N,w.shape[0])
...
def affine_backward(dout, cache):
  ...
  #############################################################################
  N=x.shape[0]
  x_reshape=x.reshape(N,np.prod(x.shape(1:)))
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
	...
	# dimension of gamma and beta should be the same as out
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  # Add padding to H,W of each image with a constant value
  ...
  for n in range(N):  # First, iterate over all the images
        for f in range(F):  # Second, iterate over all the kernels,http://bbs.fishc.com/thread-60337-1-1.html
            for k in range(Hh):
                for l in range(Hw):
                    out[n, f, k, l] = np.sum(
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
		...
		# axis=2 represents sum in z direction,3 represents sum in n different sample
...
def svm_loss(x, y):
  ...
  #np.maximum is different - it returns an array that is the element-wise maximum between two arrays.
