...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
	...
	# Step 2
    x, cache_embedding = word_embedding_forward(captions_in, W_embed)# N*T-1*W
	# Step 3, the initial state is h0 from the image
    if self.cell_type == 'rnn':
        h, cache_rnn = rnn_forward(x, h0, Wx, Wh, b)# N*T-1*H
    elif self.cell_type == 'lstm':
        h, cache_rnn = lstm_forward(x, h0, Wx, Wh, b)
    else:
        raise ValueError('%s not implemented' % (self.cell_type))
	# Step 4
    scores, cache_scores = temporal_affine_forward(h, W_vocab, b_vocab)# N*T-1*V
	# Step 5
    loss, dscores = temporal_softmax_loss(scores, captions_out, mask, verbose=False)
	
	# Backward pass，get the keys from dict params
  ...
  def sample(self, features, max_length=30):
   ...
   # Get first hidden state
	...
	for t in xrange(max_length):  # Let's go over the sequence
		...
		if self.cell_type == 'rnn':
			# Run a step of rnn，squeeze，Remove single-dimensional entries from the shape of an array.
		...
		# Compute the score distrib over the dictionary，插入维度：y[:,np.newaxis,:].shape成了(5,1,7)
